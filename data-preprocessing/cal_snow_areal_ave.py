...
days=609
file0='ERA5_daily_snowfall_rate_25N_2020_2021_combined.nc'
df0=Dataset(file0,'r')
wind0=np.array(df0.variables['snow_rate'][:])
...
#np.save('Arctic_regional_mean_snow_rate',data_mean)

#### write spatial data to nc file ####
ncfile=Dataset('ERA5_daily_mean_snow_rate_448x304_2020_2021.nc',mode='w',format='NETCDF4')
...
wind = ncfile.createVariable('snow_rate',np.float32,('time','x_dim','y_dim'))
wind.units = 'mm/day'
wind.missing_value = fill
wind.standard_name = 'Daily mean snowfall rate' # this is a CF standard name
wind[:]=data
