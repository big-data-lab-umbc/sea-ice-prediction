...
days=609
file0='ERA5_daily_surface_pressure_25N_2020_2021_combined.nc'
df0=Dataset(file0,'r')
sh0=np.array(df0.variables['sp'][:]) #Pa
era_lon=np.array(df0.variables['lon'][:])
era_lat=np.array(df0.variables['lat'][:])
sh1=sh0[0:days]/100.0 #from Pa to hPa
...
#np.save('Arctic_regional_surface_pressure',data_mean)

#### write spatial data to nc file ####
fill=-999.0
ncfile=Dataset('ERA5_daily_surface_pressure_448x304_2020_2021.nc',mode='w',format='NETCDF4')
...
wind = ncfile.createVariable('sp',np.float32,('time','x_dim','y_dim'))
wind.units = 'hPa'
wind.missing_value = fill
wind.standard_name = 'surface pressure' # this is a CF standard name
wind[:]=data
