...
### variables: near surface humidity
...
days=609
file0='ERA5_daily_sea_surface_temp_25N_2020_2021_combined.nc'
df0=Dataset(file0,'r')
sh0=np.array(df0.variables['sst'][:]) #K
era_lon=np.array(df0.variables['lon'][:])
era_lat=np.array(df0.variables['lat'][:])
sh1=sh0[0:days]
time=np.array(df0.variables['time'][:])
time1=time[0:days]
era_fill=df0.variables['sst'].missing_value
...
#np.save('Arctic_regional_mean_sea_surface_temp',data_mean)

#### write spatial data to nc file ####
fill=-999.0
data[np.where(data==era_fill)]=fill
ncfile=Dataset('ERA5_daily_sea_surface_temperature_448x304_2020_2021.nc',mode='w',format='NETCDF4')
...
wind = ncfile.createVariable('sst',np.float32,('time','x_dim','y_dim'))
wind.units = 'K'
wind.missing_value = fill
wind.standard_name = 'Sea surface temperature' # this is a CF standard name
wind[:]=data
