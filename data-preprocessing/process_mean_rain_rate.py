...
file0=path+'ERA5_hourly_mean_precip_rate_ampm_25N_2020_2021.nc'
df1=Dataset(file0,'r')
time=df1.variables['time'][:]
lon=(df1.variables['longitude'][:])
lat=(df1.variables['latitude'][:])
crr_raw=np.array(df1.variables['mtpr'][:])
#lsrr=np.array(df1.variables['lsrr'][:])
fill=df1.variables['mtpr'].missing_value
...
ncfile=Dataset('ERA5_daily_mean_rain_rate_25N_2020_2021_combined.nc',mode='w',format='NETCDF4')
...
wind = ncfile.createVariable('rain_rate',np.float32,('time','lat','lon'))
wind.units = 'mm/day'
wind.missing_value = np.nan
wind.standard_name = 'Rainfall rate' # this is a CF standard name
wind[:]=new_data
