...
file0=path+'ERA5_daily_surface_pressure_25N_2020_2021.nc'
df1=Dataset(file0,'r')
time=df1.variables['time'][:]
lon=(df1.variables['longitude'][:])
lat=(df1.variables['latitude'][:])
expver=df1.variables['expver'][:]
sst=np.array(df1.variables['sp'][:])
fill=df1.variables['sp'].missing_value
time_size=time.size
lon_size=lon.size
lat_size=lat.size
...
ncfile=Dataset('ERA5_daily_surface_pressure_25N_2020_2021_combined.nc',mode='w',format='NETCDF4')
...
wind = ncfile.createVariable('sp',np.float32,('time','lat','lon'))
wind.units = 'Pa'
